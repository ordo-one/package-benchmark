...
extension BenchmarkTool {
    ...
    private func _prettyPrint(title: String,
        ...
        if format == .markdown {
            table.print(scaledResults, style: Style.pipe)
        } else {
            table.print(scaledResults, style: Style.fancy)
        }
    }

    func prettyPrintTable(_ baseline: BenchmarkBaseline,
    ...
    func prettyPrintDeviationTable(baselineName: String,
        ...
        namesAndTargets.forEach { nameAndTarget in
            ...
            printMarkdown("```")
            "Threshold violations for \(nameAndTarget.target):\(nameAndTarget.name)".printAsHeader(addWhiteSpace: false)
            printMarkdown("```")
            ...
            metrics.forEach { metric in
                ...
                if absoluteResults.isEmpty == false {
                    ...
                    if format == .markdown {
                        absoluteTable.print(absoluteResults, style: Style.pipe)
                    } else {
                ...
                if relativeResults.isEmpty == false {
                    ...
                    if format == .markdown {
                        relativeTable.print(relativeResults, style: Style.pipe)
                    } else {
    ...
    func prettyPrintAbsoluteDeviationTable(baselineName: String,
        ...
        namesAndTargets.forEach { nameAndTarget in
            ...
            printMarkdown("```")
            "Absolute threshold violations for \(nameAndTarget.target):\(nameAndTarget.name)".printAsHeader(addWhiteSpace: false)
            printMarkdown("```")
